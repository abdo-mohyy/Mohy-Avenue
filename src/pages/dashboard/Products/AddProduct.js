...
export default function AddProduct() {
  ...
  // Handle First Submit For Form
  async function handleSubmitForm() {
    try {
      let res = await Axios.post(`${productAPI}/add`, dummyForm);
      setId(res.data.id);
    } catch (error) {
      console.log(error);
    }
  ...
  const showImages = images.map((img, key) => (
    <div key={key} className="border my-3 ">
      ...
      <div className="custom-progress bg-info w-100 mt-2">
        <span
          ref={(e) => (progressRef.current[key] = e)}
          className="inner-progress d-block bg-primary position-relative transition-3s h-100"
        ></span>
      </div>
    </div>
  ));
  ...
  return (
    <>
      {loading && <Loading />}
      <h1 className="text-center p-3 bg-info text-primary">Add Product</h1>
      <Form className="bg-info w-100 p-3" onSubmit={editForm}>
